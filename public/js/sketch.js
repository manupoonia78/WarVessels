...
function switchState(state, data){
    switch(state){
        ...
        case 'end': {
            ...
            elements.button.classList.remove('noDisplay');
            if(data == 1){
                sendWinnerToBackend(global.player.id, 1);
            }
            ...
            if(result){
                ...
                for(var i=0;i<5;i++){
                    document.getElementById(global.player.pieces[i].name+"1").classList.add('loadingImg');
                }

                if(data === 1){
                    elements.player.classList.add('noDisplay');
                    document.getElementById('won').classList.remove('noDisplay');
                    for(let j=0; j<global.opponentCanvas.board.length; j++){
                        for(let k=0; k<global.opponentCanvas.board[j].length; k++){
                            if((j===4 && (k===3 || k===6))|| (j==6 && (k==2 || k==3 || k==4 || k==5 || k==6 || k==7)) || (j==7 && (k==3 || k==4 || k==5 || k==6)) || (j==8 && (k==4 || k==5))){
                                global.opponentCanvas.board[j][k]='miss';
                            }
                            else{
                                global.opponentCanvas.board[j][k]=0;
                            }
                        }
                    }
                    ...
                    elements.player.classList.add('noDisplay');
                    document.getElementById('lose').classList.remove('noDisplay');
                    for(let j=0; j<global.opponentCanvas.board.length; j++){
                        for(let k=0; k<global.opponentCanvas.board[j].length; k++){
                            if((j===4 && (k===3 || k===6))|| (j==6 && ( k==3 || k==4 || k==5 || k==6))){
                                global.opponentCanvas.board[j][k]='miss';
                            }
                            else{
                                global.opponentCanvas.board[j][k]=0;
...
sendWinnerToBackend = (table_user_id, score_count) => {
    // fetch(`http://localhost:3000/api/v1/set_user_score`,{
    //     method: 'POST',
    //     headers: {
    ...
    //     }, 
    //     referrer: "",
    //     body: JSON.stringify({table_user_id: table_user_id, score: score_count}),
    //     })
...
function setup() {
    ...
    global.boardLength = smallSize ? 0.8 * window.innerWidth : Math.min(350, 0.8 * window.innerWidth);
