...
io.on('connection', (client) => {
	...
	client.on('attack', function (data, callback) {
		...
		if(gameExists){
				...
				if (opponent && (gameState[gameID]['lastAttack'] === null || gameState[gameID]['lastAttack']['playerID'] !== playerID || gameState[gameID]['lastAttack']['playerID'] === playerID)) {
					...
					attack[2] = hit;
					attack[3] = opponentID;
					gameState[gameID]['lastAttack'] = { playerID: playerID, attack: attack, hit: hit };
					gameState['changed'] = true;
					gameState[gameID]['players'][playerID]['attacks'].push(attack);
					client.to(gameID).emit('incomingAttack', attack);
					callback({hit: hit, error: false, opponent: opponent['playerBoard']});
			}
		}else{
...
app.get('/play/:gameID&:playerId', (req, res, next) => {
	...
	if (gameExists) {
		...
		if (playerId) {
			...
			if(gameState[gameID]['players'][playerId] !== undefined){
				...
				if(opponentID && gameState[gameID]['players'][opponentID] !== undefined){
					const opponent = gameState[gameID]['players'][opponentID];
					const player = gameState[gameID]['players'][playerId];
					var turn=gameState[gameID]['lastAttack'] ? gameState[gameID]['lastAttack']['playerID'] !== playerId : true;
					if(gameState[gameID]['lastAttack']){
						if(gameState[gameID]['lastAttack']['playerID']===playerId && gameState[gameID]['lastAttack']['hit']){
							turn=true;
						}
						if(gameState[gameID]['lastAttack']['playerID']===opponentID && gameState[gameID]['lastAttack']['hit']){
							turn=false;
						}
					}
					res.render('pages/index_multiplayer', {
						gameID: gameID,
						changed: gameState['changed'],
						playerID: playerId,
						result: player.result,
						opponent: opponent.playerBoard,
						data: player ? {
				...
				res.render('pages/index_multiplayer', { gameID: gameID,result:null, changed: gameState['changed'],playerID: playerId, data: null,opponent:null});
			}
			...
			//playerId = playerID;
			res.render('pages/index_multiplayer', { gameID: gameID,result:null, changed: gameState['changed'],playerID: ssn.playerID, data: null ,opponent:null,});
